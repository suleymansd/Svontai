...
export default function GoogleOAuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    const payload = {
      ...
      success,
      error: success ? undefined : reason || 'Google bağlantısı tamamlanamadı.',
    }
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-background px-6">
      <section className="w-full max-w-md border border-border bg-card p-8 text-center shadow-sm">
        {result === null ? (
          <p className="text-sm text-muted-foreground">Google bağlantısı doğrulanıyor...</p>
        ) : (
          <>
            {success ? (
            ...
            <h1 className="mt-4 text-xl font-semibold">
              {success ? 'Google bağlantısı tamamlandı' : 'Google bağlantısı tamamlanamadı'}
            </h1>
            <p className="mt-2 text-sm text-muted-foreground">
              {success
                ? 'Bu pencere otomatik olarak kapanacak.'
                : result.reason || 'Lütfen entegrasyon ekranından yeniden deneyin.'}
            </p>
          </>
        )}
      </section>
